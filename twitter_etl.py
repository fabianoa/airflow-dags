...
def write_to_bucket(data):
    ...
    client = Minio("minio.minio-bigdata.svc.cluster.local:9000", access_key=MINIO_ROOT_USER, secret_key=MINIO_ROOT_PASSWORD, secure=False)
    ...
    filename = (
        f"dados_brutos/tweeter/tweets/{batchDatetime.strftime('%Y/%m/%d')}/elon_tweets_{batchDatetime.strftime('%H%M%S')}_{batchId}.parquet"
    )
    client.put_object(
